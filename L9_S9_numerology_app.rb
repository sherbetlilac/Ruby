...
#1. ask user to input birthdate
puts "Enter your birthdate: 
Please enter in MMDDYYYY format"

#2. use get to assign their birthday to a variable

birthdate = gets

# 3. use array syntax to access each elemet (number) in the birthdate.
# convert each element to an integer and add them altogether.

number = birthdate[0].to_i + birthdate[1].to_i + birthdate[2].to_i + birthdate[3].to_i + birthdate[4].to_i + birthdate[5].to_i + birthdate [6].to_i + birthdate[7].to_i

# 4. convert number back to string
# convert number to intergar again to add them togeher

number = number.to_s
number = number[0].to_i + number[1].to_i

# 5. Use If statement to determine if number is > 9, if yes then reduce again
...
# 6. Use Case statement to categories each number with the respective meaning
case number
when 1
...
when 5
    puts "Your numerolofy number is #{number}. \nThis is the freedom lover. The number five is an intellectual vibration. These are 'idea' people with a love of variety and the ability to adapt to most situations. Ruled by Mercury."
when 6
